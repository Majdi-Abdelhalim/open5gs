...
if [ "$SYSTEM" = "Linux" ]; then
    # --- Loop through all 3 interfaces ---
    for IFACE in ogstun ogstun2 ogstun3; do
        
        # 1. Create interface if it doesn't exist
        if ! grep "$IFACE" /proc/net/dev > /dev/null; then
            ip tuntap add name $IFACE mode tun
        fi

        # 2. Enable IPv6 if disabled
        if test "x`sysctl -n net.ipv6.conf.$IFACE.disable_ipv6`" = x1; then
            echo "net.ipv6.conf.$IFACE.disable_ipv6=0" >> /etc/sysctl.d/30-open5gs.conf
            sysctl -p /etc/sysctl.d/30-open5gs.conf
        fi
        
        # 3. Bring interface UP (Do this before adding IPs usually, or at the end)
